...
.watchlist-manager{border:1px solid #dce5f5;border-radius:15px;background:linear-gradient(180deg,#f8faff,#fff);padding:14px;margin:12px 0 16px}.watchlist-manager-head{display:flex;justify-content:space-between;align-items:flex-start;gap:12px;margin-bottom:10px}.watchlist-manager-head b{font-size:14px;color:var(--navy)}.watchlist-manager-head span{font-size:11px;color:var(--muted);line-height:1.5;text-align:right}.watchlist-manager-form{display:flex;flex-wrap:wrap;gap:9px;align-items:center}.watchlist-sync-status[data-state="shared"]{color:var(--down)}.watchlist-sync-status[data-state="error"]{color:var(--amber)}.message-button:disabled{opacity:.6;cursor:wait}.watchlist-inline-list{display:flex;flex-wrap:wrap;align-content:flex-start;gap:8px;max-height:150px;overflow:auto;margin-top:11px;padding:9px;border:1px solid var(--line);border-radius:11px;background:#fbfcff}.watchlist-inline-item{display:inline-flex;align-items:center;gap:7px;height:32px;padding:0 6px 0 10px;border:1px solid #dce4f2;border-radius:999px;background:#fff;color:#43506a;font-size:12px;font-weight:780;white-space:nowrap}.watchlist-inline-remove{display:inline-flex;align-items:center;justify-content:center;width:22px;height:22px;border:0;border-radius:50%;background:#fff0f0;color:#b84040;font-size:17px;line-height:1;cursor:pointer}.watchlist-inline-remove:hover{background:#ffdede}.watchlist-inline-remove:disabled{opacity:.55;cursor:wait}.watchlist-empty{width:100%;padding:12px 8px;text-align:center;color:var(--muted);font-size:12px}
...
<section class="panel" id="messages">
  ...
  <div class="card">
    ...
    <div class="watchlist-manager" aria-label="共享自选股管理">
      ...
      <input id="messageCodeInput" class="message-input" placeholder="输入股票代码加入自选，如 300750">
        <button id="messageAddCode" class="message-button">新增自选股</button>
        <span class="asset-symbol">新增或删除后会立即同步，所有打开共享链接的人都能看到，并长期保留。</span>
      </div>
      <div id="messageWatchlist" class="watchlist-inline-list" aria-label="当前共享自选股"></div>
    </div>
...
async function renderMessages(){
  ...
  function renderWatchlist(){
    ...
    target.innerHTML=watchlist.length?watchlist.map(code=>{
      const name=stockNames[code]||'';
      const label=`${code}${name?` · ${name}`:''}`;
      return `<span class="watchlist-inline-item"><span>${esc(label)}</span><button class="watchlist-inline-remove" type="button" data-remove-watchlist="${esc(code)}" title="删除 ${esc(label)}" aria-label="删除 ${esc(label)}">×</button></span>`;
    }).join(''):'<div class="watchlist-empty">共享自选股目前为空，可在上方新增。</div>';
    target.querySelectorAll('[data-remove-watchlist]').forEach(button=>button.onclick=async()=>{
      const code=button.dataset.removeWatchlist||'';
      const name=stockNames[code]||'';
      if(!window.confirm(`确认删除 ${code}${name?` · ${name}`:''}？`))return;
  ...
  document.getElementById('messageAddCode').onclick=async()=>{
    ...
    const button=document.getElementById('messageAddCode');
    button.disabled=true;
    try{
  ...
  document.getElementById('messageCodeInput').onkeydown=event=>{
    if(event.key==='Enter')document.getElementById('messageAddCode').click();
  };
}
