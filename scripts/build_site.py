...
<section class="panel" id="macro"><div class="grid kpis" id="macroKpis"></div><div class="grid g2"><div class="card"><h3>M1、M2与剪刀差</h3><div id="moneyChart" class="chart"></div></div><div class="card"><h3>银行间资金利率</h3><div id="liquidityKpis" class="grid g3" style="margin-bottom:12px"></div><div id="liquidityChart" class="chart"></div></div><div class="card"><h3>美元净流动性 <small>SOMA/总资产 − TGA − RRP，单位万亿美元</small></h3><div id="macroUsdLiquidityKpis" class="grid g2" style="margin-bottom:12px"></div><div id="macroUsdLiquidityChart" class="chart"></div></div><div class="card"><h3>社会融资规模</h3><div id="socialChart" class="chart"></div></div><div class="card"><h3>中国PMI与CPI <small>制造业、非制造业与CPI同比</small></h3><div id="pmiChart" class="chart"></div></div><div class="card"><h3>美国PMI <small>S&amp;P Global制造业、服务业与50荣枯线</small></h3><div id="usPmiKpis" class="grid g2" style="margin-bottom:12px"></div><div id="usPmiChart" class="chart"></div></div><div class="card" style="grid-column:1/-1"><h3>美国PCE与联邦基金利率 <small>PCE为价格指数同比，利率为月度有效利率</small></h3><div id="pcePolicyKpis" class="grid g2" style="margin-bottom:12px"></div><div id="pcePolicyChart" class="chart"></div></div></div></section>
...
function renderUsPmi(){
  const g=DATA.global_macro;
  const manufacturing=g.filter(row=>row.series==='US_PMI_MANUFACTURING');
  const services=g.filter(row=>row.series==='US_PMI_SERVICES');
  const manufacturingLatest=latestDelta(manufacturing,'value_pct');
  const servicesLatest=latestDelta(services,'value_pct');
  const kpis=document.getElementById('usPmiKpis');
  if(kpis)kpis.innerHTML=[
    kpi('美国制造业PMI',fmt(manufacturingLatest.v),'点',noteWithDelta(manufacturingLatest.row,'trade_date',manufacturingLatest.delta),'neutral'),
    kpi('美国服务业PMI',fmt(servicesLatest.v),'点',noteWithDelta(servicesLatest.row,'trade_date',servicesLatest.delta),'neutral')
  ].join('');
  const allDates=[...manufacturing,...services].map(row=>cnDate(row.trade_date));
  const chart=document.getElementById('usPmiChart');
  if(chart&&(manufacturing.length||services.length))Plotly.newPlot('usPmiChart',[
    {x:manufacturing.map(row=>cnDate(row.trade_date)),y:manufacturing.map(row=>row.value_pct),name:'美国制造业PMI',type:'scattergl',mode:'lines+markers',connectgaps:false,line:{color:C.blue,width:2.3},marker:{size:4}},
    {x:services.map(row=>cnDate(row.trade_date)),y:services.map(row=>row.value_pct),name:'美国服务业PMI',type:'scattergl',mode:'lines+markers',connectgaps:false,line:{color:C.purple,width:2.2},marker:{size:4}}
  ],layout({margin:{l:52,r:25,t:20,b:45},yaxis:{title:'PMI点',gridcolor:C.grid},shapes:allDates.length?[{type:'line',x0:allDates.sort()[0],x1:allDates.sort().slice(-1)[0],y0:50,y1:50,line:{color:C.muted,dash:'dot'}}]:[]}),CONFIG);
}
...
function renderMacro(){
  ...
  if(m.length){
    ...
    Plotly.newPlot('pmiChart',[{x,y:m.map(row=>row.pmi_manufacturing),name:'中国制造业PMI',mode:'lines',line:{color:C.blue,width:2.3}},{x,y:m.map(row=>row.pmi_non_manufacturing),name:'中国非制造业PMI',mode:'lines',line:{color:C.cyan,width:2.1}},{x,y:m.map(row=>row.cpi_yoy_pct),name:'中国CPI同比',mode:'lines',line:{color:C.up,width:2},yaxis:'y2'}],layout({yaxis:{title:'PMI点',gridcolor:C.grid},yaxis2:{title:'CPI同比（%）',overlaying:'y',side:'right',showgrid:false},shapes:[{type:'line',x0:x[0],x1:x[x.length-1],y0:50,y1:50,line:{color:C.muted,dash:'dot'}}]}),CONFIG);
  }
  renderUsPmi();
  renderPcePolicy();
